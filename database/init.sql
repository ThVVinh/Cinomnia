...
CREATE OR ALTER TRIGGER [dbo].[tg_transaction_update_updateOwnedMovie]
ON [dbo].[transaction]
AFTER INSERT, DELETE
AS
BEGIN TRANSACTION
    ...
    INSERT INTO owned_movie (user_id, movie_id)
    SELECT i.user_id, td.movie_id
    FROM inserted i
    JOIN [dbo].[transaction_detail] td ON td.transaction_id = i.id
